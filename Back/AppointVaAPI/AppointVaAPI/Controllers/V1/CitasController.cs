    ...
    public class CitasController : ControllerBase
    {
        ...
        public async Task<IActionResult> Crear([FromBody] CrearCitaDto dto)
        {
            ...
            _db.NotificacionesDashboard.Add(new NotificacionDashboard
            {
                ...
                Descripcion = $"{servicio.Nombre} con {creada!.Empleado?.Nombre ?? "Empleado"} · {cita.InicioEn.ToString("ddd d 'de' MMM, h:mm tt", new System.Globalization.CultureInfo("es-MX"))}",
                CitaId = cita.Id
            });
            ...
            using (var tx = await _db.Database.BeginTransactionAsync())
            {
                ...
                if (dto.NuevoEstado == EstadosCitas.Cancelada)
                {
                    _db.NotificacionesDashboard.Add(new NotificacionDashboard
                    {
                        ...
                        Descripcion = $"{cita.Servicio?.Nombre ?? "Servicio"} con {cita.Empleado?.Nombre ?? "Empleado"} · {cita.InicioEn.ToString("ddd d 'de' MMM, h:mm tt", new System.Globalization.CultureInfo("es-MX"))}",
                        CitaId = cita.Id
                    });
        ...
        public async Task<IActionResult> Reagendar(Guid id, [FromBody] ReagendarCitaDto dto)
        {
            ...
            var inicio15UtcReag = AppointVaAPI.Helpers.ZonaHorariaHelper.ToDateTimeOffset(cita.InicioEn, tzReag);
            if (inicio15UtcReag.AddMinutes(-15) > DateTimeOffset.UtcNow)
                _jobClient.Schedule<IPushService>(
                    s => s.EnviarRecordatorio15MinAsync(cita.Id),
                    inicio15UtcReag.AddMinutes(-15));
