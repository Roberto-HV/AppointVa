      ...
      <div className="w-full max-w-md">
        ...
        <div className="mt-5 space-y-3 print:hidden">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
            <p className="text-xs font-semibold text-amber-800 mb-1.5">Guarda el link de tu cita</p>
            <div className="flex items-center gap-2">
              <span className="flex-1 text-xs text-amber-700 font-mono truncate bg-white border border-amber-200 rounded-lg px-2 py-1.5">
                {linkCita}
              </span>
              <button
                onClick={copiarLink}
                className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-700 hover:bg-amber-800 text-white text-xs font-semibold transition"
              >
                {linkCopiado ? <Check size={12} /> : <Copy size={12} />}
                {linkCopiado ? "¡Copiado!" : "Copiar"}
              </button>
            </div>
            <p className="text-xs text-amber-600 mt-1.5">Desde aquí podrás consultar y cancelar tu cita en cualquier momento.</p>
          </div>

          {/* Guardar comprobante */}
          <button
            onClick={() => window.print()}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gray-800 hover:bg-gray-700 text-white text-sm font-semibold transition"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#25D366] hover:bg-[#1ebe5d] text-white text-sm font-semibold transition"
          >
          ...
          {(cita.icalUrl || cita.googleCalUrl) && (
            <div className="flex gap-2">
              {/* Apple Calendar — webcal:// abre la app directamente */}
          ...
          {cancelable && !confirmandoCancelar && puedeCancel && (
            <button
              ...
              className="w-full py-2.5 rounded-xl border-2 border-red-200 text-red-500 text-sm font-medium hover:border-red-300 transition"
            >
              Cancelar cita
            </button>
          )}
